...
export function AuthModal({ isOpen, onClose, initialMode = 'signin' }: AuthModalProps) {
  ...
  const [role, setRole] = useState<'customer' | 'food_truck_owner'>('customer');
  ...
  const handleGoogleSignIn = async (useFirebase = false) => {
    setError('');
    const handleGoogleSignIn = async () => {
      try {
        setLoading(true);
        setError('');
        await signInWithGoogle(true); // true for Firebase Google Auth
        onClose();
      } catch (err: any) {
          ...
          {mode === 'signup' && (
            <>
              ...
              <div>
                ...
                <select
                  value={role}
                  onChange={(e) => setRole(e.target.value as 'customer' | 'food_truck_owner')}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                >
          ...
          <>
            ...
            <div className="space-y-3 w-full">
              <button
                ...
                disabled={loading}
                className="w-full flex items-center justify-center gap-3 bg-white border-2 border-gray-300 text-gray-700 py-2.5 rounded-lg hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed font-medium transition-colors"
              >
